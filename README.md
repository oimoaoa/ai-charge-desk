...
| **비용 ($, ₩)** | `ccusage`로 계산한 **추정 토큰 비용**(API 단가 환산). **구독 청구액이 아님** — 정액 구독이면 실제 결제액과 무관한 참고치. 원화는 USD→KRW 환율(frankfurter.app) 환산 참고값. **비용은 사용률(%)보다 늦게 반영될 수 있음** — 사용률 %는 실시간 조회지만, 비용은 CLI가 대화(세션)를 **끝낸 뒤** 남기는 로컬 로그를 집계하므로, 새로고침해도 방금 쓴 비용이 아직 안 뜰 수 있습니다(세션이 끝나고 다음 수집에 따라잡음). 버그가 아니라 정상이며 결국 정확히 반영됩니다. |
