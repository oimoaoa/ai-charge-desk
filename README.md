...
| **비용 ($, ₩)** | `ccusage`로 계산한 **추정 토큰 비용**(API 단가 환산). **구독 청구액이 아님** — 정액 구독이면 실제 결제액과 무관한 참고치. 원화는 USD→KRW 환율(frankfurter.app) 환산 참고값. **비용은 Claude Code·Codex(개발 도구) 사용만 집계** — Codex는 CLI·데스크탑 앱, Claude는 Claude Code(CLI·IDE) 사용이 잡힙니다. **claude.ai·chatgpt.com의 웹/데스크탑 채팅은 대상이 아닙니다**(개발 도구 사용을 API 비용으로 환산하는 도구라 정상). 또 로컬 세션 로그는 대화(세션)가 끝난 뒤 쌓이므로, **사용률 %**(계정 실시간이라 경로 무관 전부 반영)보다 한 박자 늦게 반영될 수 있습니다(다음 수집에 따라잡음). 버그가 아니라 정상입니다. |
...
- 개발을 몰라도 됩니다 — Claude Code(또는 Codex) 대화방에 "AI Charge Desk 업데이트해줘"라고 부탁하면 위 절차를 대신 해줍니다.
...
- `WARN`(70%+ 주의) · `DANGER`(90%+ 위험) · `INK`(본문) · `SUBTLE`(비용·데이터 나이) · `CLAUDE`/`CODEX`(서비스 헤더).
...
> "SUBTLE 색을 더 진하게 바꿔줘"처럼 부탁하면 알아서 고쳐줍니다.
