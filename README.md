...
Claude·Codex 앱은 "남은 사용량"을 각자 앱 안에서만 보여줍니다. 이 도구는 두 서비스의 사용률을 **쓴 비율(used %) 기준으로 통일**해 메뉴바에 상시 표시하고, 토큰 비용 추정치를 원화로 환산해 같이 보여줍니다. 모든 데이터는 로컬에만 저장됩니다.

## 각 숫자의 뜻

| 항목 | 뜻 |
| --- | --- |
| **사용률 (%)** | 해당 창에서 **쓴 비율**. 100%에 가까울수록 한도 임박. 앱이 보여주는 "남음 %"의 반대 방향이니 주의. Claude·Codex 모두 각 서비스의 실시간 사용량 API에서 읽어와 앱과 같은 값. |
...
| **이번 달 / 이번 주 / 오늘** | 매월 1일(KST)~ / 월요일~일요일 / 오늘 0시~. 각 패널 우상단에 실제 집계 구간 표기. 사용률 %의 리셋 창과는 별개 기준. |
...
- **읽기 전용**: 저장된 access token으로 사용량을 조회만 합니다. 토큰을 갱신(refresh)하거나 쓰지 않으므로 **Claude/Codex 로그인이 깨지지 않습니다.**
...
- **Claude 쪽만 안 나옴**: 두 가지 원인 — ① 키체인 접근을 거부한 경우: `node scripts/build-snapshot.mjs`를 터미널에서 실행해 키체인 창에서 "항상 허용"을 누르세요. ② Claude Code를 오래 안 써 토큰이 만료된 경우: Claude Code를 한 번 쓰면 되살아납니다(그동안은 마지막 성공값과 그 수집 시각을 함께 표시).
