> ## ⚠️ 이 저장소는 보관(archive) 상태입니다 — 더 이상 유지보수하지 않습니다
>
> **2026-08-18부터 읽기 전용**입니다. 마지막 코드 업데이트는 **2026-07-26**이고, 그 뒤로는 관리하지 못했습니다.
>
> - **동작을 보장하지 않습니다**. 이 도구는 Claude·Codex의 **비공식 사용량 API**에 기대고 있어서, 그쪽이 바뀌면 예고 없이 값이 안 나오거나 틀린 값이 나올 수 있습니다(원래 알려진 제약 — [이슈 #2](https://github.com/oimoaoa/ai-charge-desk/issues/2)). 현재 그 API가 그대로인지는 확인하지 않았습니다.
> - **표시되는 비용을 신뢰하지 마세요**. 원래도 토큰 기준 추정치이지 실제 청구액이 아니었고, 유지보수가 멈춘 지금은 모델·단가 표가 낡았을 수 있습니다.
> - **이슈·PR은 받지 않습니다** — 보관 상태라 잠깁니다.
> - **가져다 쓰는 건 자유입니다**. MIT 라이선스 그대로라 fork 떠서 고쳐 쓰셔도 됩니다.
>
> 아래 문서는 **2026-07-26 시점 기준 기록**으로 남겨둡니다. 지금 동작과 다를 수 있습니다.
...
> **새로고침 완료 신호:** 드롭다운의 **데이터 새로고침**이나 **토큰 갱신하기**를 누르면 처리 과정에서 드롭다운이 먼저 닫힐 수 있습니다. **드롭다운이 닫힌 것은 완료 신호가 아닙니다.** 수집과 반영이 끝나 상태바의 AI Charge Desk 항목이 한 번 깜빡이면 완료입니다. 아직 깜빡이지 않았다면 실패로 단정하지 말고 조금 더 기다려 주세요.
...
- **AI Charge Desk 항목 자체가 메뉴바에서 사라짐**:
    1. 카메라 노치가 있는 Mac에서 오른쪽 아이콘이 많거나 녹음·음악 재생 같은 **임시 상태 아이콘**이 생기면, macOS가 공간이 부족한 항목 전체를 잠시 숨길 수 있습니다. 이때도 SwiftBar와 플러그인은 실행 중일 수 있습니다.
    2. 녹음·재생을 끝내 공간이 돌아와도 즉시 나타나지 않고 **다음 SwiftBar 화면 갱신(최대 2분)** 때 늦게 돌아올 수 있습니다. 잠시 기다리거나 SwiftBar의 `Refresh All`을 실행하세요.
    3. 갱신 뒤에도 없으면 응용 프로그램에서 **SwiftBar를 다시 여세요**. 이미 실행 중이었다면 새로 시작하는 대신 화면 갱신을 촉발할 수 있습니다. 항목이 안 보인다는 사실만으로 앱이 종료됐다고 단정할 수는 없습니다.
